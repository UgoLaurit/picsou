...
export const AmountCell = ({ value, row, onAmountChange }: AmountCellProps) => {
  ...
  const isSubcategoryRow = Boolean(row.original.subcategory)
  ...
  return (
    <div
      onClick={() => isSubcategoryRow && setIsEditing(true)}
      className={cn(
        'text-right px-2 py-0.5 rounded',
        isSubcategoryRow && 'cursor-pointer hover:bg-muted',
